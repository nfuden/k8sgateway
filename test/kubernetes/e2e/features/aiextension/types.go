...
var (
	...
	backendPassthroughManifest = filepath.Join(
		fsutils.MustGetThisDir(),
		"testdata",
		"backend-passthrough.yaml",
	)
	...
	routesWithExtensionManifest = filepath.Join(
		fsutils.MustGetThisDir(),
		"testdata",
		"routes-with-extension-ref.yaml",
	)

	// routes to LLM backends with extension ref for prompt guard regex pattern rejection
	routesWitPGRegexPatternRejectManifest = filepath.Join(
		fsutils.MustGetThisDir(),
		"testdata",
		"routes-with-pg-regex-pattern-reject.yaml",
	)

	// routes options for streaming
	routeOptionStreamingManifest = filepath.Join(
		fsutils.MustGetThisDir(),
		"testdata",
		"streaming.yaml",
	)
	...
	trafficPolicyPGRegexPatternRejectManifest = filepath.Join(
		fsutils.MustGetThisDir(),
		"testdata",
		"prompt-guard-regex-pattern-reject.yaml",
	)
	...
	promptGuardStreamingManifest = filepath.Join(
		fsutils.MustGetThisDir(),
		"testdata",
		"prompt-guard-streaming.yaml",
	)
)
