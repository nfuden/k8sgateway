...
### Plugin
For this initial pass this would be entirely at the listener level and would have its own plugin. 
Given the concerns for extensibility it makes sense to not bind this tighly to the other listeneroption policies so that this functionality can be overriden.
