...
spec:
  ...
  options:
    extauth:
    - provider: basic-extauth
      merge:
        sectionName: auth
        placement: During
---
## Opt In to extauth2 and opt out of the blanket top level extauth
apiVersion: gateway.kgateway.dev/v1alpha1
kind: RoutePolicy
metadata:
  name: myroute
spec:
  ...
  options:
    extauth:
    - provider: basic-extauth-2
      merge: ## if not set default to auth
        sectionName: auth
        placement: During
    - provider: basic-extauth 
      disable: true ## maybe not in first pass
...
### Plugin
For this initial pass we should implement Providers and the basic reference on routepolicy. We may want to not implement route level disablement in the first pass.
Given the concerns for extensibility that we have some type of generalized Provider type for external destinations such as extauth and tracing.
