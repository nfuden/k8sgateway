...
func (s *serviceEntryCollections) resolveServiceEntryBackendRef(
	...
	results := krt.Fetch(
		kctx,
		s.Backends,
		krt.FilterIndex(s.backendsBySourceObj, serviceEntryKey(key)),
	)
...
func (s *serviceEntryCollections) resolveHostnameBackendRef(
	...
	results := krt.Fetch(
		kctx,
		s.Backends,
		krt.FilterIndex(s.backendsByHostPort, makeHostPortKey(hostname, int(port))),
	)
	for _, res := range results {
		if out == nil ||
			res.Obj.GetCreationTimestamp().Time.Before(out.Obj.GetCreationTimestamp().Time) {
			out = &res
		}
	}
