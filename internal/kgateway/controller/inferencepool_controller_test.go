...
var _ = Describe("InferencePool controller", func() {
	...
	Context("when Inference Extension deployer is enabled", func() {
		...
		It(
			"should reconcile an InferencePool referenced by a managed HTTPRoute and deploy the endpoint picker",
			func() {
								...
								BackendRefs: []apiv1.HTTPBackendRef{
									{
										BackendRef: apiv1.BackendRef{
											BackendObjectReference: apiv1.BackendObjectReference{
												Group: ptr.To(
													apiv1.Group(infextv1a2.GroupVersion.Group),
												),
				...
				Eventually(func() error {
					return k8sClient.Get(
						ctx,
						client.ObjectKey{Namespace: defaultNamespace, Name: expectedName},
						&deploy,
					)
		...
		It(
			"should ignore an InferencePool not referenced by any HTTPRoute and not deploy the endpoint picker",
			func() {
				...
				Consistently(func() error {
					...
					return k8sClient.Get(
						...
						client.ObjectKey{
							Namespace: defaultNamespace,
							Name:      fmt.Sprintf("%s-endpoint-picker", pool.Name),
						},
						&dep,
					)
