...
func snapshotPerClient(
	...
	xdsSnapshotsForUcc := krt.NewCollection(
		uccCol,
		func(kctx krt.HandlerContext, ucc ir.UniqlyConnectedClient) *XdsSnapWrapper {
			...
			l.Debug(
				"found perclient clusters",
				zap.String("client", ucc.ResourceName()),
				zap.Int("clusters", len(clustersForUcc)),
			)
			...
			if len(clustersForUcc) == 0 {
				l.Info(
					"no perclient clusters; defer building snapshot",
					zap.String("client", ucc.ResourceName()),
				)
				return nil
			}
			...
			clustersProto := make(
				[]envoycachetypes.ResourceWithTTL,
				0,
				len(clustersForUcc)+len(maybeMostlySnap.Clusters),
			)
			var clustersHash uint64
			var erroredClusters []string
			for _, c := range clustersForUcc {
				if c.Error == nil {
					clustersProto = append(
						clustersProto,
						envoycachetypes.ResourceWithTTL{Resource: c.Cluster},
					)
			...
			for _, ep := range endpointsForUcc {
				endpointsProto = append(
					endpointsProto,
					envoycachetypes.ResourceWithTTL{Resource: ep.Endpoints},
				)
			...
			endpointResources := envoycache.NewResourcesWithTTL(
				fmt.Sprintf("%d", endpointsHash),
				endpointsProto,
			)
